...
<head>
  ...
  <title>Quản lý bài học JavaScript</title>
...
<div class="admin-container">
    <div class="page-header">
        {{-- Sửa icon, tiêu đề và route cho JavaScript --}}
        <h1><i class="fab fa-js-square"></i> Quản lý bài học JavaScript</h1>
        <div>
            <a href="{{ route('admin.lesson') }}" class="back-btn" style="margin-right: 15px;"><i class="fas fa-arrow-left"></i> Quay lại</a>
            <a href="{{ route('admin.javascript.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Thêm bài học</a>
        </div>
    </div>
    ...
    <table class="content-table">
      ...
      <tbody>
        {{-- Sửa biến lặp cho JavaScript --}}
        @forelse($javascript as $lesson)
        <tr>
          ...
          <td>
            <div class="table-actions">
                ...
                {{-- Sửa route cho JavaScript --}}
                <a href="{{ route('admin.javascript.edit', $lesson->lesson_id) }}" class="btn btn-edit">
                  <i class="fas fa-edit"></i> Sửa
                </a>
                <form action="{{ route('admin.javascript.destroy', $lesson->lesson_id) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa?');">
                  @csrf
                  @method('DELETE')
