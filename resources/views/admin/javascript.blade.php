...
<body>
  ...
  <div class="container">
    <h1>Quản lý bài học Javascript</h1>
    <a href="{{ route('admin.javascript.create') }}" class="btn-action btn-add" style="margin-bottom:18px; display:inline-block;">
    ...
    <table class="lesson-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <a href="{{ route('admin.javascript.edit', $lesson->lesson_id) }}" class="btn-action btn-edit" title="Chỉnh sửa">
              <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle;">
                <path d="M12.5 3.5l2 2a2 2 0 010 2.8l-7.5 7.5a2 2 0 01-1.4.6H3v-2.1a2 2 0 01.6-1.4l7.5-7.5a2 2 0 012.8 0z"/>
              </svg>
              Sửa
            </a>
            <form action="{{ route('admin.javascript.destroy', $lesson->lesson_id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Bạn có chắc chắn muốn xóa?');">
              @csrf
              @method('DELETE')
