...
<div class="container py-4">
    ...
    <form action="{{ route('quiz.update', $quiz) }}" method="POST">
        ...
        <a href="{{ route('quiz.index') }}" class="btn btn-secondary">Quay lại</a>
    </form>

    <form action="{{ route('quiz.destroy', $quiz) }}" method="POST" class="mt-4" onsubmit="return confirm('Bạn chắc chắn muốn xóa câu hỏi này?');">
        @csrf
        @method('DELETE')
